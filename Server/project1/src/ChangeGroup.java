...
public class ChangeGroup extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            System.out.println("the receive json is:changegroup" + AccaptJson);
            if (AccaptJson != null) {
                JSONObject AcJson = JSONObject.fromObject(AccaptJson);
                String UserName = AcJson.getString("username");
                String FriendName = AcJson.getString("friendname");
                String NewGroup = AcJson.getString("newgroup");
                operator.changeGroup(UserName, FriendName, NewGroup);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

...
public class UpdateNews extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            System.out.println("the receive json is:updatenews" + AccaptJson);
            if (AccaptJson != null) {
                JSONObject AcJson = JSONObject.fromObject(AccaptJson);
                String UserName = AcJson.getString("username");
                String Name = AcJson.getString("name");
                String ImgPath = AcJson.getString("imgpath");
                String News = AcJson.getString("news");
                NewsModel user = new NewsModel(News, UserName, Name, ImgPath);
                operator.updateNews(user);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
